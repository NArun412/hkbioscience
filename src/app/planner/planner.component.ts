...
export class PlannerComponent implements OnInit {
  ...
  constructor(private plannerService: PlannerService, private blogPostService: BlogPostService, private router: Router, private sharedService: SharedService) { }
...
ngOnInit(): void {
  ...
  Submit() {
    ...
    if ((this.selectedOption === 'email' && this.email && this.projectStartTime) ||
      ...
      this.plannerService.submitPlannerData(plannerData).subscribe(
        response => {
          Swal.fire({
            toast: true,
            position: 'top',
  ...
  sendEmail(plannerData: any) {
    this.plannerService.sentemail(plannerData).subscribe(
      (res: any) => {
        if (res.result) {
          Swal.fire({
            toast: true,
